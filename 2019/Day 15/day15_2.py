...
def find_oxygen_system(state, pos, seen, curr_dist, map):
    ...
    for dir in range(1,5):
        ...
        if next_pos not in seen:
            ...
            if move_result == 2:
                ...
                best_solution = min(best_solution, find_oxygen_system(state_copy, next_pos, seen, curr_dist+1, map))
                map[next_pos] = '.'
            else:
                map[next_pos] = '#'
...
def fill_oxygen_step(map):
    areas_changed = 0
    changed = set()
    for pos in map.keys():
        if pos in changed or map[pos] != 'O':
            continue
        ...
        for dir in range(1,5):
            ...
            if map[next_pos] == '.':
                map[next_pos] = 'O'
                changed.add(next_pos)
                areas_changed += 1
    return areas_changed
        

def fill_area_with_oxygen(map):
    empty_areas = 0
    oxygen_tank = None
    for key in map.keys():
        if map[key] == '.':
    ...
    steps = 0
    while empty_areas > 0:
        areas_changed = fill_oxygen_step(map)
        empty_areas -= areas_changed
        steps += 1
...
find_oxygen_system(initial_state, (0,0), seen, 0, map)
print(fill_area_with_oxygen(map))
